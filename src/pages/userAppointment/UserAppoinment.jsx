...
export default function AppointmentsPage() {
  ...
  const confirmCancelAppointment = async () => {
    ...
    try {
      ...
      setAppointments(
        appointments.map((apt) => (apt.id === selectedAppointment.id ? { ...apt, status: "cancelled" } : apt)),
      )
  ...
  const upcomingAppointments = appointments.filter(
    (apt) => !isPast(new Date(`${apt.date}T${apt.time}`)) && (apt.status === "accepted" || apt.status === "scheduled"),
  )

  const pendingAppointments = appointments.filter((apt) => apt.status === "pending")

  const pastAppointments = appointments.filter(
    (apt) => isPast(new Date(`${apt.date}T${apt.time}`)) || ["rejected", "cancelled", "completed"].includes(apt.status),
  )
                                      ...
                                      <CardContent className="p-6">
                                          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                              <div className="flex items-start gap-4">
                                                  ...
                                                  <div>
                                                      <h3 className="font-medium text-lg">Session with {appointment.therapistName}</h3>
                  ...
                  <TabsContent value="pending">
                      {pendingAppointments.length > 0 ? (
                                              ...
                                              <div className="flex items-start gap-4">
                                                  ...
                                                  <div>
                                                      ...
                                                      <p className="mt-2 text-sm text-muted-foreground">
                                                          Waiting for therapist to confirm this appointment request.
                                                      </p>
                                                  </div>
                                              </div>
                                              <div className="flex flex-col sm:flex-row gap-2 mt-4 md:mt-0">
                          ...
                          <div className="text-center py-12 bg-muted/30 rounded-lg">
                              ...
                              <p className="text-muted-foreground mb-6">You don't have any pending appointment requests</p>
                              <Button asChild>
                                  <Link to="/therapists">Find a Therapist</Link>
                              </Button>
                          </div>
                                              ...
                                              <div className="flex items-start gap-4">
                                                  ...
                                                  <div>
                                                      <div className="flex items-center gap-2">
                                                          <h3 className="font-medium text-lg">Session with {appointment.therapistName}</h3>
                                                          <Badge
                                                              variant={getStatusBadgeVariant(appointment.status, appointment.date, appointment.time)}
                                                          >
                                                              {getStatusLabel(appointment.status, appointment.date, appointment.time)}
                                                          </Badge>
                                                      </div>
                                                      <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3 text-muted-foreground mt-1">
                                                          <div className="flex items-center">
                                              ...
                                              <div className="flex flex-col sm:flex-row gap-2 mt-4 md:mt-0">
                                                  ...
                                                  {appointment.status !== "cancelled" && appointment.status !== "rejected" && (
                                                      <Button variant="secondary" size="sm" asChild>
                                                          <Link to={`/therapists/${appointment.therapistId}`}>Book Again</Link>
                                                      </Button>
                                                  )}
                                              </div>
                                          </div>
              ...
              <Dialog open={cancelDialogOpen} onOpenChange={setCancelDialogOpen}>
                  <DialogContent>
                      <DialogHeader>
                          ...
                          <DialogDescription>
                              Are you sure you want to cancel your appointment with {selectedAppointment?.therapistName} on{" "}
