...
export default function TherapistAppointmentsPage() {
  ...
  const todayAppointments = appointments.filter(
    (apt) => isToday(new Date(`${apt.date}T${apt.time}`)) && (apt.status === "accepted" || apt.status === "scheduled"),
  );
                    ...
                    <CardContent className="p-6">
                      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div>
                            <h3 className="font-medium text-lg">Session with {appointment.patientName}</h3>
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 mt-4 md:mt-0">
                          <Button variant="outline" size="sm" asChild>
                            <Link to={`/therapist-dashboard/messages/${appointment.patientId}`}>
            ...
            <div className="grid grid-cols-7 gap-2">
              {weekDays.map((day) => (
                <div key={day.toString()} className="border rounded-md p-2">
                  <div
                    className={`text-center p-2 rounded-md mb-2 ${isToday(day) ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                  >
